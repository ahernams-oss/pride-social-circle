...
BEGIN
  ...
  IF _conversation_id IS NULL THEN
    ...
    VALUES (_conversation_id, _caller_id);
...
DROP POLICY IF EXISTS "approved add conversation recipient" ON public.conversation_participants;
CREATE POLICY "approved add conversation recipient"
...
WITH CHECK (
  ...
  AND EXISTS (
    ...
    WHERE target.id = conversation_participants.user_id
